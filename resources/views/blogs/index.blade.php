    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    @if (session()->has('blogMessages'))
                    <div class="p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg dark:bg-blue-200 dark:text-blue-800" role="alert">
                        <span class="font-medium">Info! </span>{{ session()->get('message') }}
                    </div>
                    @endif
                    <a href="{{ route('blogs.create') }}" class="py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Add new blog</a>
                                ...
                                <tr class="bg-white border-b light:bg-gray-900 light:border-gray-700">
                                    ...
                                    <td class="py-4 px-6 flex">
                                        ...
                                        <form method="POST" action="{{ route('blogs.destroy', $blog->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{ route('blogs.destroy', $blog->id) }}" class="font-medium text-blue-600 light:text-blue-500 hover:underline ml-2 confirm_image_delete">Delete</a>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
