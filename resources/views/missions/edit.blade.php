<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Missions') }}</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="overflow-x-auto relative shadow-md sm:rounded-lg m-auto">
                        <form action="{{ route('missions.update', $mission->id) }}" method="POST" class="w-96" style="margin: 0 auto;" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-6">
                                <div>
                                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900 light:text-gray-300">Title</label>
                                    <input type="text" id="title" name="title" value="{{ $mission->title }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500" placeholder="e.g. Lorem Ipsum.." required>
                                    @error('title')
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="text" name="text" rows="6" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500" placeholder="Text..">{{ $mission->text }}</textarea>
                                @error('text')
                                <p class="mt-2 text-sm text-red-500 light:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <img class="w-50 h-50 rounded" src="/storage/{{ $mission->image }}" />
                            </div>
